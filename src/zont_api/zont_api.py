...
class ZontAPIException(Exception):
    ...
    def __init__(self, message=None, status_code=None, description=None):
        ...
        self.status_code = status_code if status_code else ZontAPIException.__status_code__
...
class ZontAPI:
    ...
    def __init__(self, token=None, client=None):
        ...
        logger.info('%s initialized for client %s', self.api_headers["User-Agent"], self.api_client)
    ...
    def convert_delta_time_array(self, delta_time_array, sort=True, reverse=False):
        ...
        if not isinstance(delta_time_array, list):
            raise ValueError(f'parent: list expected but found {type(delta_time_array).__name__}')
        ...
        for element in delta_time_array:
            if not isinstance(element, list):
                raise ValueError(f'element: list expected but found {type(element).__name__}')
...
class ZontDevice:
    ...
    def __init__(self, device_data: dict = None, api: ZontAPI = None):
        ...
        self.last_seen = self.data.get('last_receive_time')
